...
export class FieldVariableGetter extends (Blockly.FieldVariable as any) {
     ...
     */
    saveState() {
        const legacyState = this.saveLegacyState(FieldVariableGetter);
        if (legacyState !== null) {
            return legacyState;
        }
        // Make sure the variable is initialized.
        ...
        if (this.variable_) {
            state['id'] = this.variable_.getId();
            state['name'] = this.variable_.name;
            state['type'] = this.variable_.type;
        } else if (this.selectedOption_) {
            state['id'] = this.selectedOption_[0];
            state['name'] = this.selectedOption_[1];
            state['type'] = this.defaultType_;
        }
        return state;
    }
     ...
     * @override
     */
    loadState(state: Record<string, string>) {
